...
namespace Scaleform
{
	...
	void TrueHUDMenu::AddRecentLootMessage(std::string_view a_name, uint32_t a_count, const char* a_iconLabel, uint32_t a_iconColor, const QuickLoot::Items::ItemStack* a_stack) const
	{
		if (_recentLoot) {
			_recentLoot->AddMessage(a_name, a_count, a_iconLabel, a_iconColor, a_stack);
		}
	}
	...
	void TrueHUDMenu::DrawSphere(const RE::NiPoint3& a_origin, float a_radius, uint32_t a_segments /*= 16*/, float a_duration /*= 0.f*/, uint32_t a_color /*= 0xFF0000FF*/, float a_thickness /*= 1.f*/)
	{
		...
		a_segments = std::max(a_segments, minSegments);
